...
# ── TensionBudget recording routes (separate from Chords CSV recording) ──

@app.route('/tb/start_recording', methods=['POST'])
def tb_start_recording():
    """
    Start TensionBudget-format CSV recording.
    This is separate from the standard Chords recording and produces
    an enhanced CSV with timestamps, Arduino counter, and session metadata.
    Returns:
        JSON response with recording status and CSV filename.
    """
    global connection_manager
    if not connection_manager or not connection_manager.stream_active:
        return jsonify({'status': 'error', 'message': 'No active stream'}), 400
    ...
    csv_path = connection_manager.start_tb_recording(output_dir=output_dir)
    if csv_path:
        post_console_message(f"TB recording started: {csv_path}")
        return jsonify({'status': 'tb_recording_started', 'csv_file': str(csv_path)})
    return jsonify({'status': 'error', 'message': 'Failed to start TB recording'}), 500

@app.route('/tb/stop_recording', methods=['POST'])
def tb_stop_recording():
    """
    Stop TensionBudget-format CSV recording and write session manifest.
    Returns:
        JSON response with manifest path and recording summary.
    """
    global connection_manager
    if not connection_manager:
        return jsonify({'status': 'error', 'message': 'No active connection'}), 400

    manifest = connection_manager.stop_tb_recording()
    if manifest:
        post_console_message(f"TB recording stopped. Manifest: {manifest}")
        return jsonify({'status': 'tb_recording_stopped', 'manifest': str(manifest)})
    return jsonify({'status': 'error', 'message': 'No active TB recording'}), 400

@app.route('/tb/status')
def tb_recording_status():
    """
    Get TensionBudget recording status (sample count, drops, elapsed time).
    Returns:
        JSON response with TB recording status dict.
    """
    global connection_manager
    if connection_manager:
        return jsonify(connection_manager.get_tb_status())
