...
class Connection:
    ...
    def __init__(self):
        ...
        # TensionBudget recording (separate from Chords CSV recording)
        self.tb_recorder = None                    # TBRecorder instance when TB recording is active
        ...
        # Sample callback system (used by TBRecorder and other extensions)
        self.sample_callbacks = []                 # List of callables: f(channel_data, timestamp, arduino_counter)
    ...
    # ── TensionBudget recording (separate from Chords CSV) ─────────────

    def start_tb_recording(self, output_dir=None):
        """
        Start TensionBudget-format CSV recording (separate from Chords recording).

        This creates a TBRecorder that registers as a sample callback,
        writing an enhanced CSV with timestamps, Arduino counter, and
        session metadata. Both Chords and TB recording can run simultaneously.

        Args:
            output_dir: Directory for TB output files. If None, uses CWD.

        Returns:
            Path to the TB CSV file, or None on failure.
        """
        if self.tb_recorder and self.tb_recorder.recording:
            print("TB recording is already active.")
            return None

        try:
            from chordspy.tensionbudget.recorder import TBRecorder
            self.tb_recorder = TBRecorder(output_dir=output_dir)
            csv_path = self.tb_recorder.start(self)
            return csv_path
        except Exception as e:
            print(f"Error starting TB recording: {e}")
            self.tb_recorder = None
            return None

    def stop_tb_recording(self):
        """
        Stop TensionBudget-format CSV recording.

        Returns:
            Path to the session manifest JSON, or None.
        """
        if not self.tb_recorder:
            return None

        try:
            manifest = self.tb_recorder.stop()
            return manifest
        except Exception as e:
            print(f"Error stopping TB recording: {e}")
            return None
        finally:
            self.tb_recorder = None
    ...
    def get_tb_status(self):
        """
        Get TensionBudget recording status.

        Returns:
            dict with recording state, sample count, drop count, etc.
        """
        if self.tb_recorder:
            return self.tb_recorder.get_status()
    ...
    def register_sample_callback(self, callback):
        """
        Register a callback to receive every sample as it is acquired.

        The callback signature must be:
            callback(channel_data: list[float], timestamp: float, arduino_counter: int | None)

        - channel_data: list of channel values (floats)
        - timestamp: pylsl.local_clock() timestamp for the sample (monotonic,
          high-resolution). Note: TBRecorder captures its own time.perf_counter()
          timestamp independently for CSV recording.
        - arduino_counter: 0-255 rolling counter from the Arduino packet,
          or None if the protocol doesn't provide one (WiFi/BLE)

        Callbacks are called synchronously from the data handler thread.
        Keep them fast to avoid blocking the acquisition loop.

        Args:
            callback: Callable with the signature above.
        """
                ...
                if hasattr(self.usb_connection, 'ser') and self.usb_connection.ser.is_open:
                    ...
                    if hasattr(self.usb_connection, 'data'):
                        ...
                        if current_time >= next_sample_time:
                            ...
                            # Notify registered callbacks (e.g. TBRecorder)
                            arduino_counter = getattr(self.usb_connection, 'last_counter', None)
                            for cb in self.sample_callbacks:
                                try:
                                    cb(channel_data, sample_time, arduino_counter)
                                except Exception as cb_err:
                                    print(f"Sample callback error: {cb_err}")
            except Exception as e:
    ...
    def cleanup(self):
        ...
        self.stop_tb_recording()     # Stop TensionBudget recording if active
